...
function StoreDashboard({ session, onLogout }) {
  ...
  const advanceStatus = async (order) => {
    ...
    try {
      ...
      if (!res.ok) return;
      const updated = await res.json();
      // The PATCH response won't include rider_name (it's only added by
      // the /api/stores/orders join), so merge onto the existing row
      // rather than replacing it wholesale.
      setOrders((os) =>
        os.map((o) => (o.id === updated.id ? { ...o, ...updated } : o)),
      );
    } catch {
      // silent — next poll will resync
    }
