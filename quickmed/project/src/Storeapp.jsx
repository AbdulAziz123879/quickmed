...
import {
  Store, LogOut, Eye, EyeOff, RefreshCw, Package, Wallet, Bell, Check, Clock, X,
} from "lucide-react";
import { C } from "./theme";
...
export default function StoreApp() {
  ...
  if (!session) {
    return <StoreLogin onLogin={(token, store) => setSession({ token, store })} />;
  }
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0B1220", fontFamily: "'Inter', sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 380, background: "#111A2B", border: "1px solid #1E293B", borderRadius: 20, padding: 36 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 22 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: `linear-gradient(135deg, ${C.primary}, ${C.secondary})`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Store size={20} color="#fff" />
          </div>
          <div>
            <div style={{ fontSize: 17, fontWeight: 800, color: "#F1F5F9" }}>Store Partner Portal</div>
            <div style={{ fontSize: 12, color: "#94A3B8" }}>For Quick Med pharmacy partners</div>
          </div>
        </div>
        ...
        <div style={{ fontSize: 11.5, color: "#94A3B8", marginTop: 18, lineHeight: 1.6 }}>
          Don't have store login details? Ask a Quick Med admin to add your store from the admin panel — that's where your email and password are set up.
        </div>
      </div>
    </div>
  );
...
function StoreDashboard({ session, onLogout }) {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/stores/orders`, { headers: { "x-store-token": token } });
      const data = await res.json().catch(() => ({}));
  ...
  const advanceStatus = async (order) => {
    const idx = STATUS_FLOW.findIndex((s) => s.toLowerCase() === (order.status || "").toLowerCase());
    const next = STATUS_FLOW[Math.min(idx + 1, STATUS_FLOW.length - 1)];
    if (!next || next === order.status) return;
    try {
      const res = await fetch(`${API_BASE_URL}/api/stores/orders/${order.id}/status`, {
        method: "PATCH",
  ...
  const logout = async () => {
    try {
      await fetch(`${API_BASE_URL}/api/stores/logout`, { method: "POST", headers: { "x-store-token": token } });
    } catch {}
    onLogout();
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B1220", color: "#F1F5F9", fontFamily: "'Inter', sans-serif" }}>
      {/* Top bar */}
      <div style={{ borderBottom: "1px solid #1E293B", padding: "16px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 9, background: `linear-gradient(135deg, ${C.primary}, ${C.secondary})`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Store size={17} color="#fff" />
          </div>
          <div>
            <div style={{ fontSize: 15.5, fontWeight: 800 }}>{store.name}</div>
            <div style={{ fontSize: 11.5, color: "#94A3B8" }}>Store partner portal</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={fetchOrders} style={iconBtn} aria-label="Refresh"><RefreshCw size={16} /></button>
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 32 }}>
              <StatCard icon={Bell} label="New orders" value={newOrders.length} tone="rgba(239,68,68,0.14)" iconColor="#F87171" highlight={newOrders.length > 0} />
              <StatCard icon={Package} label="Total orders" value={totalOrders} tone="rgba(37,99,235,0.14)" iconColor="#60A5FA" />
              <StatCard icon={Wallet} label="Total sales" value={`৳${totalSales.toLocaleString()}`} tone="rgba(16,185,129,0.14)" iconColor="#34D399" />
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <div style={{ fontSize: 15, fontWeight: 700, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                New orders {newOrders.length > 0 && <span style={{ background: "#F87171", color: "#fff", fontSize: 11, fontWeight: 800, borderRadius: 999, padding: "2px 8px" }}>{newOrders.length}</span>}
              </div>
              {newOrders.length > 0 && (
                <button onClick={markAllSeen} style={{ background: "none", border: `1px solid #1E293B`, color: "#94A3B8", padding: "7px 14px", borderRadius: 999, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                  Mark all as seen
                </button>
              )}
            </div>
            ...
            {newOrders.length === 0 ? (
              <div style={{ background: "#111A2B", border: "1px solid #1E293B", borderRadius: 14, padding: 22, textAlign: "center", fontSize: 13, color: "#94A3B8", marginBottom: 36 }}>
                No new orders right now — this checks for new ones automatically every few seconds.
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 36 }}>
                {newOrders.map((o) => (
                  <OrderRow key={o.id} order={o} isNew onAdvance={advanceStatus} />
                ))}
              </div>
            )}

            {/* All orders */}
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>All orders</div>
            {orders.length === 0 ? (
              <div style={{ textAlign: "center", padding: "40px 0", color: "#94A3B8", fontSize: 13.5 }}>No orders yet.</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {orders.map((o) => (
                  <OrderRow key={o.id} order={o} isNew={o.id > lastSeenId} onAdvance={advanceStatus} />
                ))}
              </div>
            )}
          </>
...
function StatCard({ icon: Icon, label, value, tone, iconColor, highlight }) {
  return (
    <div style={{ background: "#111A2B", border: `1px solid ${highlight ? "#F87171" : "#1E293B"}`, borderRadius: 16, padding: 18 }}>
      <div style={{ width: 36, height: 36, borderRadius: 10, background: tone, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>
...
function OrderRow({ order, isNew, onAdvance }) {
  ...
  return (
    <div style={{
      background: "#111A2B", border: `1px solid ${isNew ? "#F87171" : "#1E293B"}`, borderRadius: 14,
      ...
      <div style={{ flex: 1, minWidth: 220 }}>
        <div style={{ fontSize: 13.5, fontWeight: 700, display: "flex", alignItems: "center", gap: 8 }}>
          {order.id} <span style={{ color: "#94A3B8", fontWeight: 500 }}>· {order.order_date}</span>
