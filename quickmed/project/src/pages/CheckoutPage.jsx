...
/* CheckoutPage.jsx
   ...
   Checkout now works off a fixed `items` list — the exact set of cart
   items the customer checked on the Shopping Cart page — rather than
   the entire cart. Anything left unchecked never appears here and stays
   in the cart untouched. After a successful order, only these items are
   removed from the cart (by id).
*/
...
export function CheckoutPage({
  ...
  const total = subtotal + (items.length ? 25 : 0) + Math.round(subtotal * 0.05);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      orderedIds.forEach((id) => removeFromCart?.(id));
      setPlaced(true);
    } catch (e) {
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                opacity: placing || items.length === 0 ? 0.7 : 1,
              }}
            >
