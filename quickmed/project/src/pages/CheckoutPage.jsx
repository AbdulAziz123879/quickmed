...
/* CheckoutPage.jsx
   Checkout form for delivery details and payment.

   Now captures the customer's name and delivery address and sends them
   along when placing the order, so the store and rider dashboards have
   something real to show instead of blank fields.
*/
import { useState } from "react";
import { MapPin, CreditCard, Upload, Check } from "lucide-react";
import { C, inputStyle } from "../theme";
...
export function CheckoutPage({ theme, cart, goTo, addToCart }) {
  const [placed, setPlaced] = useState(false);
  const [placing, setPlacing] = useState(false);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      await api.createOrder({
        id: `QM-${Math.floor(10000 + Math.random() * 89999)}`,
        items: itemsText,
        total,
      ...
      setPlaceError(e.message || "Couldn't place your order. Please try again.");
    } finally {
      setPlacing(false);
    }
  };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 32, marginTop: 32 }} className="qm-checkout-grid">
        <Reveal>
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 16, padding: 22 }}>
              <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}><MapPin size={16} color={C.primary} /> Delivery address</div>
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name" style={inputStyle(theme)} />
              <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="House / road / area" style={{ ...inputStyle(theme), marginTop: 10 }} />
              <input value={city} onChange={(e) => setCity(e.target.value)} placeholder="City" style={{ ...inputStyle(theme), marginTop: 10 }} />
            </div>
            <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 16, padding: 22 }}>
              <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}><CreditCard size={16} color={C.primary} /> Payment method</div>
              {["Credit / debit card", "Mobile banking (bKash/Nagad)", "Cash on delivery"].map((p, i) => (
        ...
        <Reveal delay={100}>
          <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 18, padding: 24, position: "sticky", top: 90 }}>
            ...
            <div style={{ borderTop: `1px solid ${theme.border}`, marginTop: 10, paddingTop: 14, display: "flex", justifyContent: "space-between", fontWeight: 800, fontSize: 16 }}><span>Total</span><span>৳{total}</span></div>
            {placeError && <div style={{ fontSize: 12.5, color: C.danger, fontWeight: 600, marginTop: 14 }}>{placeError}</div>}
            <button
              onClick={handlePlaceOrder}
              disabled={placing || cart.length === 0}
              className="qm-btn"
              style={{
                width: "100%", marginTop: 20, background: C.primary, color: "#fff", border: "none",
                padding: "13px", borderRadius: 12, fontWeight: 700, fontSize: 14.5,
