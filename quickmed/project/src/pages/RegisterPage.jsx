...
export function RegisterPage({ theme, goTo, onCustomerLogin }) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const profile = await api.registerCustomer({ name, email, phone, password: pw });
          ...
          {role === "customer" ? (
            <>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address" style={inputStyle(theme)} />
                <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone number" style={inputStyle(theme)} />
                <input type="password" value={pw} onChange={(e) => setPw(e.target.value)} placeholder="Create password" style={inputStyle(theme)} />
                {pw.length > 0 && (
                  <div>
                    <div style={{ height: 4, borderRadius: 999, background: theme.border, overflow: "hidden" }}>
                ...
                <button
                  ...
                  disabled={submitting}
                  className="qm-btn"
                  style={{ background: C.primary, color: "#fff", border: "none", padding: "13px", borderRadius: 12, fontWeight: 700, fontSize: 14.5, cursor: submitting ? "default" : "pointer", marginTop: 8, opacity: submitting ? 0.7 : 1 }}
                >
