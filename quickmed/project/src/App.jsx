...
//                 ...NAV_ITEMS,
//                 ...(customer
//                   ? [{ label: "Dashboard", page: "dashboard" }, { label: "Cart", page: "cart" }, { label: `Log out (${customer.name?.split(" ")[0] || "Account"})`, action: "logout" }]
...
//       {/* LOGIN PROMPT TOAST — shown when a logged-out visitor tries to add to cart */}
//       {loginPrompt && (
//         <div style={{ position: "fixed", bottom: 28, left: "50%", transform: "translateX(-50%)", zIndex: 500, background: dark ? "#111A2B" : "#111827", color: "#fff", padding: "14px 20px", borderRadius: 14, boxShadow: "0 20px 48px -16px rgba(0,0,0,0.4)", display: "flex", alignItems: "center", gap: 14, maxWidth: "calc(100vw - 32px)" }}>
//           <span style={{ fontSize: 13.5, fontWeight: 600 }}>Please log in to add items to your cart.</span>
//           <button
//             onClick={() => { setLoginPrompt(false); goTo("login"); }}
//             style={{ background: C.primary, color: "#fff", border: "none", padding: "8px 16px", borderRadius: 999, fontSize: 12.5, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}
//           >
//             Log in
//           </button>
//           <button onClick={() => setLoginPrompt(false)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.6)", cursor: "pointer", padding: 0, display: "flex" }} aria-label="Dismiss">
//             <X size={16} />
//           </button>
//         </div>
//       )}
...
export default function QuickMedApp() {
  ...
  else if (page === "checkout") body = <CheckoutPage theme={theme} cart={cart} goTo={goTo} addToCart={addToCart} clearCart={clearCart} />;
  else if (page === "login") body = <LoginPage theme={theme} goTo={goTo} onRiderLogin={handleRiderLogin} onCustomerLogin={handleCustomerLogin} />;
  else if (page === "register") body = <RegisterPage theme={theme} goTo={goTo} onCustomerLogin={handleCustomerLogin} />;
  else if (page === "dashboard") body = <DashboardPage theme={theme} cart={cart} wishlist={wishlist} goTo={goTo} addToCart={addToCart} customer={customer} onLogout={handleCustomerLogout} />;
  else if (page === "tracking") body = <TrackingPage theme={theme} />;
